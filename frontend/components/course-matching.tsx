...
export function CourseMatching({ transferData, onReset }: CourseMatchingProps) {
  ...
  useEffect(() => {
    const get_courses = async () => {
      ...
      const uniqueCourses = Array.from(new Map(res.data[0].map((c) => [c.course_code, c])).values())
      const credits = uniqueCourses.reduce((sum, course) => sum + course.credits, 0)
      ...
      {transferData.courses.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center gap-5 justify-end">
            ...
            <h1 className="text-5xl font-semibold text-foreground">{totalCredits} / 60</h1>
          </div>
            ...
            {transferData.courses.map((course, index) => (
              ...
              >
                <div className="flex gap-3 justify-between w-full items-center">
                  ...
                  <p className="font-medium text-foreground text-center flex-1">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full mt-2">
            {transferData.review.map((course, index) => (
              <div
                key={index}
                className="flex justify-between bg-white border-2 border-black p-4 rounded-lg shadow-md transition-all duration-300 hover:shadow-xl hover:scale-105 h-full items-center"
              >
